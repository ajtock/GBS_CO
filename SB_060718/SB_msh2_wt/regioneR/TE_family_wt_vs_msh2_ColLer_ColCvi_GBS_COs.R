...
# overlapping elements in different TE families

# Usage:
# /applications/R/R-3.3.2/bin/Rscript TE_family_wt_vs_msh2_ColLer_ColCvi_GBS_COs.R "Col/Ler and Col/Cvi GBS crossover intervals" HS_wt_ColLer_F2_2018 SB_msh2_ColLer_F2_2018 SB_wt_ColCvi_F2_2018 SB_msh2_ColCvi_F2_2018 "Col/Ler" "Col/Cvi" wt msh2 10000 
...
load(paste0("permTest_", pop1_geno1, "_COs_vs_TEsDNA.RData"))
pop1_geno1_pt_DNA <- ptCOsTEsDNAPerChrom
ptCOsTEsDNAPerChrom <- NULL
load(paste0("permTest_", pop1_geno1, "_COs_vs_TEsRNA.RData"))
pop1_geno1_pt_RNA <- ptCOsTEsRNAPerChrom
ptCOsTEsRNAPerChrom <- NULL
pop1_geno1_pt <- c(pop1_geno1_pt_DNA, pop1_geno1_pt_RNA)
 
load(paste0("permTest_", pop1_geno2, "_COs_vs_TEsDNA.RData"))
pop1_geno2_pt_DNA <- ptCOsTEsDNAPerChrom
ptCOsTEsDNAPerChrom <- NULL
load(paste0("permTest_", pop1_geno2, "_COs_vs_TEsRNA.RData"))
pop1_geno2_pt_RNA <- ptCOsTEsRNAPerChrom
ptCOsTEsRNAPerChrom <- NULL
pop1_geno2_pt <- c(pop1_geno2_pt_DNA, pop1_geno2_pt_RNA)

load(paste0("permTest_", pop2_geno1, "_COs_vs_TEsDNA.RData"))
pop2_geno1_pt_DNA <- ptCOsTEsDNAPerChrom
ptCOsTEsDNAPerChrom <- NULL
load(paste0("permTest_", pop2_geno1, "_COs_vs_TEsRNA.RData"))
pop2_geno1_pt_RNA <- ptCOsTEsRNAPerChrom
ptCOsTEsRNAPerChrom <- NULL
pop2_geno1_pt <- c(pop2_geno1_pt_DNA, pop2_geno1_pt_RNA)

load(paste0("permTest_", pop2_geno2, "_COs_vs_TEsDNA.RData"))
pop2_geno2_pt_DNA <- ptCOsTEsDNAPerChrom
ptCOsTEsDNAPerChrom <- NULL
load(paste0("permTest_", pop2_geno2, "_COs_vs_TEsRNA.RData"))
pop2_geno2_pt_RNA <- ptCOsTEsRNAPerChrom
ptCOsTEsRNAPerChrom <- NULL
pop2_geno2_pt <- c(pop2_geno2_pt_DNA, pop2_geno2_pt_RNA)
...
pop1_geno1_pt_famNames_sorted <- famNames[sort.int(unlist(pop1_geno1_pt_log2ObsExp), decreasing = T, index.return = T)$ix]
pop1_geno1_pt_famNamesPlot_sorted <- famNamesPlot[sort.int(unlist(pop1_geno1_pt_log2ObsExp), decreasing = T, index.return = T)$ix]
...
pop1_geno2_pt_famNames_sorted <- famNames[sort.int(unlist(pop1_geno1_pt_log2ObsExp), decreasing = T, index.return = T)$ix]
pop1_geno2_pt_famNamesPlot_sorted <- famNamesPlot[sort.int(unlist(pop1_geno1_pt_log2ObsExp), decreasing = T, index.return = T)$ix]
...
pop2_geno1_pt_famNames_sorted <- famNames[sort.int(unlist(pop1_geno1_pt_log2ObsExp), decreasing = T, index.return = T)$ix]
pop2_geno1_pt_famNamesPlot_sorted <- famNamesPlot[sort.int(unlist(pop1_geno1_pt_log2ObsExp), decreasing = T, index.return = T)$ix]
...
pop2_geno2_pt_famNames_sorted <- famNames[sort.int(unlist(pop1_geno1_pt_log2ObsExp), decreasing = T, index.return = T)$ix]
pop2_geno2_pt_famNamesPlot_sorted <- famNamesPlot[sort.int(unlist(pop1_geno1_pt_log2ObsExp), decreasing = T, index.return = T)$ix]

df <- data.frame(Sample = rep(c(pop1_geno1, pop1_geno2, pop2_geno1, pop2_geno2),
                              each = length(pop1_geno1_pt_log2ObsExp_sorted)),
                 Annotation_feature = rep(pop1_geno1_pt_famNamesPlot_sorted, 2),
...
df$Annotation_feature <- factor(df$Annotation_feature,
                                levels = pop1_geno1_pt_famNamesPlot_sorted)
