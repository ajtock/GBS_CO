...
# csmit -m 20G -c 1 "/applications/R/R-3.4.0/bin/Rscript crossover_Profiles_commandArgs.R 2000 2kb 20 /projects/ajt200/BAM_masters/SPO11-oligo/WT/coverage/log2ChIPinput/log2wtSPO11oligoRPI1NakedDNA_norm_allchrs_coverage_coord_tab.bed SPO11_1_oligos_RPI1"
...
COs <- read.table("/projects/ajt200/GBS_CO/CL_asy1_271119/asy1hom/asy1hom.cos.txt",
                  header = T)
COsGR <- GRanges(seqnames = paste0("Chr", COs$chr),
                 ranges = IRanges(start = COs$cos-1,
                                  end = COs$cos+1),
                 strand = "*")
print(length(COsGR))
...
outDF <- list(paste0(matDir, libName,
                     "_norm_cov_feature_smoothed_target_midpoint_and_", flankName, "_flank_dataframe.txt"),
              paste0(matDir, libName,
                     "_norm_cov_ranLoc_smoothed_target_midpoint_and_", flankName, "_flank_dataframe.txt"))
outDFcolMeans <- list(paste0(matDir, libName,
                             "_norm_cov_feature_smoothed_target_midpoint_and_", flankName, "_flank_dataframe_colMeans.txt"),
                      paste0(matDir, libName,
                             "_norm_cov_ranLoc_smoothed_target_midpoint_and_", flankName,"_flank_dataframe_colMeans.txt"))
