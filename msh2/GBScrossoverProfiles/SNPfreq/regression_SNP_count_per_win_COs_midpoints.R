...
# Tabulate results
genotype_ZINB_midWin_df <- data.frame("Window" = winName,
                                      "Estimate" = as.vector(coef(genotype_ZINB[[midWin]])),
                                      "2.5_CI" = as.vector(confint(genotype_ZINB[[midWin]])[,1]),
                                      "97.5_CI" = as.vector(confint(genotype_ZINB[[midWin]])[,2]),
                                      "Std_error" = c(as.vector(coef(summary(genotype_ZINB[[midWin]]))$count[,2])[1:2],
                                                      coef(summary(genotype_ZINB[[midWin]]))$zero[,2]),
                                      "Z-value" = c(as.vector(coef(summary(genotype_ZINB[[midWin]]))$count[,3])[1:2],
                                                    coef(summary(genotype_ZINB[[midWin]]))$zero[,3]),
                                      "P-value" = c(as.vector(coef(summary(genotype_ZINB[[midWin]]))$count[,4])[1:2],
                                                    coef(summary(genotype_ZINB[[midWin]]))$zero[,4]))
...
write.table(genotype_ZINB_midWin_df,
            ...
            row.names = T, col.names = T, sep = "\t", quote = T)
