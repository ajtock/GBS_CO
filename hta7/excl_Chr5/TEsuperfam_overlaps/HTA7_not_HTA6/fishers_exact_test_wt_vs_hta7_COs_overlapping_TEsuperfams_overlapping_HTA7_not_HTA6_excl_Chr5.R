...
# Usage: 
# /applications/R/R-4.0.0/bin/Rscript fishers_exact_test_wt_vs_hta7_COs_overlapping_TEsuperfams_overlapping_HTA7_not_HTA6_excl_Chr5.R coller.filt coller.hta7 wt hta7 "Pericentromeric crossovers" peri
...
chrStart <- c(rep(1, 4))
chrLens <- c(30427671, 19698289, 23459830, 18585056)
centromeres <- c(15086045, 3607929, 13587786, 3956021)
# Pericentromeric regions are as defined in Supplemental Table S26
# of Ziolkowski et al. (2017) Genes Dev. 31
pericenStart <- c(11330001, 990001, 10200001, 990001)
pericenEnd <- c(18480000, 7540000, 16860000, 6850000)
...
pop1COs <- read.table(paste0("/projects/ajt200/GBS_CO/hta7/excl_Chr5/COs/", pop1Name, "cos.txt"),
                      header = T)
...
pop1COsGR_pooled <- pop1COsGR_pooled[seqnames(pop1COsGR_pooled) %in% chrs]
...
pop2COs <- read.table(paste0("/projects/ajt200/GBS_CO/hta7/excl_Chr5/COs/", pop2Name, "cos.txt"),
                      header = T)
...
pop2COsGR_pooled <- pop2COsGR_pooled[seqnames(pop2COsGR_pooled) %in% chrs]
# Convert library (individual) names (e.g., from "1.1" to "1.01") to enable
# sorting individuals by increasing number
...
TEsDNAGR <- lapply(seq_along(DNAfamNames), function(x) {
  ...
  TEsDNAGR_all <- GRanges(seqnames = TEsDNA$chr,
                          ...
                          strand = "*")
  TEsDNA_HTA7_not_HTA6_overlaps <- findOverlaps(query = HTA7_not_HTA6,
                                                subject = TEsDNAGR_all,
                                                ignore.strand = TRUE,
                                                select = "all")
  TEsDNAGR_all <- TEsDNAGR_all[unique(subjectHits(TEsDNA_HTA7_not_HTA6_overlaps))]
...
TEsRNAGR <- lapply(seq_along(RNAfamNames), function(x) {
  ...
  TEsRNAGR_all <- GRanges(seqnames = TEsRNA$chr,
                          ...
                          strand = "*")
  TEsRNA_HTA7_not_HTA6_overlaps <- findOverlaps(query = HTA7_not_HTA6,
                                                subject = TEsRNAGR_all,
                                                ignore.strand = TRUE,
                                                select = "all")
  TEsRNAGR_all <- TEsRNAGR_all[unique(subjectHits(TEsRNA_HTA7_not_HTA6_overlaps))]
...
bp <- ggplot(data = df,
  ...
  ggtitle(paste0(plotTitle, " (excluding Chr5)")) +
  annotate(geom = "text",
           size = 6,
           x = c(0.8, 1.2,
                 1.8, 2.2,
...
ggsave(paste0(plotDir, "barplot_proportion_of_COs_in_",
              pop1NamePlot, "_and_", pop2NamePlot, "_overlapping_TEsuperfams_that_overlap_HTA7_not_HTA6",
              "_BH_adjusted_P_",
              region, "_excl_Chr5.pdf"),
       plot = bp,
       height = 8, width = 10)
