...
for(i in 1:5) {
  ...
  dat <- cbind(cum_windows, norm_win_wtco)
  cum_co_dat <- rbind(cum_co_dat, dat)

  # define windows2 as GRanges object
  windows2 <- seq(1, chr_ends[i], by = 200000)
  windows2 <- c(windows2, chr_ends[i])
...
write.table(cum_co_dat, file = paste0(gbs_dir, "wtcuco_genome_10kb.txt"))
write.table(cum_meth_dat, file = paste0(gbs_dir, "wtmeth_genome_200kb.txt"))

test <- seq(1, 1000, by = 1)
j = 500
ma <- rep(1, test[j])/test[j]
filt_wtco <- stats::filter(cum_co_dat[,2], ma)
which_na <- which(is.na(filt_wtco) == TRUE)
left_na <- which_na[which(which_na < 250)]
left_val <- filt_wtco[left_na[length(left_na)]+1]
filt_wtco[left_na] <- left_val
right_na <- which_na[which(which_na > 250)]
right_val <- filt_wtco[right_na[1]-1]
filt_wtco[right_na] <- right_val
filt_wtco_n <- filt_wtco[!is.na(filt_wtco)]
filt <- cbind(cum_co_dat[,1], filt_wtco)
write.table(filt, file = paste0(gbs_dir, "filt_wtcuco_genome_10kb.txt"))

j2 = 5
ma2 <- rep(1, test[j2])/test[j2]
filt_wtCG <- stats::filter(cum_meth_dat[,2], ma2); filt_wtCG_n <- filt_wtCG[!is.na(filt_wtCG)]
filt_wtCHG <- stats::filter(cum_meth_dat[,3], ma2); filt_wtCHG_n <- filt_wtCHG[!is.na(filt_wtCHG)]
filt_wtCHH <- stats::filter(cum_meth_dat[,4], ma2); filt_wtCHH_n <- filt_wtCHH[!is.na(filt_wtCHH)]

filt_wtMeth <- cbind(cum_meth_dat[,1], filt_wtCG, filt_wtCHG, filt_wtCHH)
write.table(filt_wtMeth, file = paste0(gbs_dir, "filt_wtMeth_genome_200kb.txt"))

pdf(paste0(plot_dir, "wtco_10kb_wtmeth_200kb_genome_v270417.pdf"), height = 4, width = 10)
par(mfrow = c(1, 1))
