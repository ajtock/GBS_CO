...
# Usage:
# ./bargraph_permTest_ovule_hypermeth_vs_other.R "Hypermethylated loci in ovules" " " ovule_hypermeth_loci ovule_hypermeth_loci 10000

library(ggplot2)
library(ggthemes)

#plotTitle <- "Hypermethylated loci in ovules"
#pt1Name <- " "
#pt1LibName <- "ovule_hypermeth_loci" 
#ptOrderLibName <- "ovule_hypermeth_loci"
...
otherNames <- c("COs", "RowanCOs", "SPO11", "nucleRnucs")
otherNamesPlot <- c("Serra/Underwood COs", "Rowan COs", "SPO11-1-oligo hotspots", "Well-positioned nucleosomes")
...
load(paste0(ptOrderDir, "permTest_", perms, "perms_",
            ptOrderLibName, "_vs_others.RData"))
...
bp <- ggplot(data = df,
  ...
  labs(y = expression("Log"[2]*"(observed/expected) loci overlap")) +
