...
for(k in 1:length(lib.nums)){
        print(k)
        dat <- read.table(file=paste0(inDir,"n96.HS.Recq.HEI10.series1",k,".bchqsnvmask.smooth.co.txt",sep=""),header=F)
        all <- NULL
        for(i in 1:5){
                print(i)
...
for(k in 1:length(lib.nums)){
        print(k)
        dat <- read.table(file=paste0(inDir,"n96.HS.Recq.HEI10.series2",k,".bchqsnvmask.smooth.co.txt",sep=""),header=F)
        all <- NULL
        for(i in 1:5){
                print(i)
                chr <- dat[which(dat[,2]==i),]
