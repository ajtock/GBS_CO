...
targetsGR <- GRanges(seqnames = paste0("Chr", COs$chr),
                     ranges = IRanges(start = COs$block1.end.pos,
                                      end = COs$block2.start.pos),
                     strand = "*")
targetsGR <- unique(targetsGR)
print("***********COs***********")
...
pdf(paste0(plotDir, "GBS_crossovers_base_relative_frequency_diffYaxes_v170619.pdf"), height = 5, width = 6)
par(mfrow = c(2, 2))
...
mergeBaseFreqPlotDiffY(at.coords = target.ATfreq, gc.coords = target.GCfreq,
                       ...
                       mainTitle1 = bquote("GBS crossovers ("*italic("n")*" = "*.(length(targetsGR))*")"),
                       mainTitle2 = bquote("Random loci ("*italic("n")*" = "*.(length(targetsGR))*")"))
...
pdf(paste0(plotDir, "GBS_crossovers_peak_base_relative_frequency_v170619.pdf"), height = 5, width = 6)
par(mfrow = c(2, 2))
par(mar = c(2.1, 3.2, 2.1, 3.2))
par(mgp = c(2.25, 1, 0))
flankSize <- 5000
...
pdf(paste0(plotDir, "GBS_crossovers_peak_base_relative_frequency_AT_and_GC_diffYlims_v170619.pdf"), height = 10, width = 6)
par(mfrow = c(4, 2))
par(mar = c(2.1, 3.2, 2.1, 3.2))
par(mgp = c(2.25, 1, 0))
flankSize <- 5000
